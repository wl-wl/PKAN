...
class KAN(nn.Module):
    ...
    def initialize_from_another_model(self, another_model, x):
        ...
        >>> print(model_fine.act_fun[0].coef[0][0].data2)
        ...
        for l in range(self.depth):
            ...
            # print(spb.mask.data2, self.act_fun[l].mask.data2)
    ...
    def update_grid_from_samples(self, x):
        ...
        -------
        >>> model = KAN(width=[2,5,1], grid=5, k=3)
        >>> print(model.act_fun[0].grid[0].data2)
    ...
    def initialize_grid_from_another_model(self, model, x):
        ...
        >>> model_parent.act_fun[0].grid.data2 = torch.linspace(-2,2,steps=6)[None,:]
        >>> x = torch.linspace(-2,2,steps=1001)[:,None]
        >>> model = KAN(width=[1,1], grid=5, k=3)
        >>> print(model.act_fun[0].grid.data2)
